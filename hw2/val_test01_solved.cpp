...
//
{
  ...
  x = ( int * ) malloc ( (n+1) * sizeof ( int ) ); //BUG: here x[0]-x[n] need (n+1)*sizeof(double), not (n)*sizeof(double)
  ...
  free(x); // BUG: malloc should be paired with free, rather than delete []
